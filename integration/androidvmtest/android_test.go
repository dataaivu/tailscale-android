...
//go:embed testdata/NetprobeReceiver.java
var netprobeReceiverJava []byte
...
`), 0644); err != nil {
	...
	javaSrcDir := filepath.Join(dir, "java", "com", "tailscale", "ipn", "integrationprobe")
	...
	javaSrc := filepath.Join(javaSrcDir, "NetprobeReceiver.java")
	if err := os.WriteFile(javaSrc, netprobeReceiverJava, 0644); err != nil {
		t.Fatal(err)
	}
	...
	runCmd(t, root, "javac", "-source", "8", "-target", "8", "-bootclasspath", androidJar(t), "-d", classesDir, javaSrc)
	d8Args := append([]string{"--min-api", "24", "--lib", androidJar(t), "--output", dexDir}, classFiles(t, classesDir)...)
